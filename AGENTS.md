...
See the one-liner at the top of this file for the product.

This repo also uses [AI Blueprint](https://github.com/aiblueprinthq/ai-blueprint)
as a file-backed workflow overlay (plans, specs, review gates). Blueprint does
not define the product stack. Never run a framework scaffolder (`create-next-app`,
SvelteKit, etc.) inside this directory; the extension scaffold already exists.
...
- `blueprint/context/project-overview.md` - durable product context from the plans (coding SoT remains `.cursor/rules/`)
